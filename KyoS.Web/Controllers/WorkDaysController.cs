...
namespace KyoS.Web.Controllers
{
    ...
    public class WorkDaysController : Controller
    {
        ...
        public WorkDaysController(DataContext context, ICombosHelper combosHelper, IConverterHelper converterHelper, IDateHelper dateHelper, IRenderHelper renderHelper)
        {
            _context = context;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CreateModal(WeekViewModel entity)
        {
            if (ModelState.IsValid)
            {
                string[] workdays;
                ...
                if (!string.IsNullOrEmpty(entity.Workdays))
                {
                    ...
                    {
                        await _context.SaveChangesAsync();
                        List<WeekEntity> week_List = await _context.Weeks
                                                                   .Include(n => n.Clinic)
                                                                   .Include(n => n.Days)
                                                                   .ToListAsync();

                        return Json(new { isValid = true, html = _renderHelper.RenderRazorViewToString(this, "_ViewWorkDays", week_List) });
                    }
                    catch (System.Exception ex)
                    {
                        if (ex.InnerException.Message.Contains("duplicate"))
            ...
            return Json(new { isValid = false, html = _renderHelper.RenderRazorViewToString(this, "CreateModal", entity) });
        }
